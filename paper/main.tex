...
\hypersetup{
  ...
  citecolor=black,
  urlcolor=blue,
  pdftitle={Feature and Neighbor Ensembles in Locally Adaptive Clustering and Stochastic Neighbor Embedding: Response Functions and Perplexity Tolerance},
...
\title{\textbf{Feature and Neighbor Ensembles in Locally Adaptive Clustering and Stochastic Neighbor Embedding: Response Functions and Perplexity Tolerance}}
...
\begin{abstract}
Locally Adaptive Clustering (LAC) and Stochastic Neighbor Embedding (SNE) define normalized exponential distributions on feature and neighbor state spaces, respectively. We use this distinction to derive and test response diagnostics rather than treating Gibbs normalization itself as a contribution. Across 30 realizations of a structurally heterogeneous synthetic design, a two-pass rule that selects one global LAC temperature from the initial response and then freezes it attains an adjusted Rand index (ARI) of $0.983\pm0.008$, compared with $0.988\pm0.006$ for a retrospective oracle, $0.972\pm0.010$ for fixed $h=1$, and $0.842\pm0.024$ for $k$-means initialization. Recomputing $h$ at every iteration gives $0.982\pm0.008$ and exhibits no cycles in the 30 runs, while independently selected cluster temperatures perform substantially worse. On standardized Iris and Breast Cancer Wisconsin Diagnostic data, the global response rules improve on fixed-$h$ LAC, whereas on Wine they do not exceed $k$-means, providing a mixed real-data validation. For SNE, the canonical response gives a first-order conversion between bandwidth error and perplexity error; on Wine the median response implies that a 1\% perplexity tolerance corresponds to approximately 0.35\% relative bandwidth accuracy. Deterministic annealing on the same synthetic geometry exhibits an assignment bifurcation, whereas fixed LAC feature spectra display smooth finite crossovers. The normalized-exponential identities are treated throughout as established prior structure.
\end{abstract}
...
\caption{Principal symbols used in the statistical-mechanical formulation. Algorithm-specific quantities are introduced again at their first detailed use.}
\label{tab:symbols}
\begin{tabularx}{\textwidth}{>{\centering\arraybackslash}p{0.16\textwidth} X}
\toprule
...
$p$ & Probability vector on a finite state space; $p\in\Delta_{m-1}$.\\
...
\section{State-space comparison and interpretation}
\label{sec:unified}
...
\begin{proposition}[State-space specialization of the classical Gibbs variational principle]
\label{prop:common_entropy_principle}
We restate the classical Gibbs variational principle in the form needed for the comparison. Let $\mathcal S=\{s_1,\ldots,s_m\}$ be a finite state space, let $E_a\in\mathbb R$ be a finite energy assigned to state $s_a$, and let $T>0$. On $\Delta_{m-1}$, consider
\begin{equation}
    \mathcal J_T[p]
    :=\sum_{a=1}^{m}p_aE_a
      +T\sum_{a=1}^{m}p_a\ln\left(p_a\right)
    =U[p]-TS[p].
    \label{eq:common_variational_functional}
\end{equation}
Its unique minimizer is
\begin{equation}
    p_a^\star
    =\frac{\exp\left(-E_a/T\right)}
    {\displaystyle\sum_{b=1}^{m}\exp\left(-E_b/T\right)}.
    \label{eq:common_variational_minimizer}
\end{equation}
The LAC feature distribution and SNE neighbor distribution are obtained by the specializations
\begin{subequations}
\label{eq:common_recoveries}
\begin{empheq}[left=\empheqlbrace]{align}
    \text{LAC:}\quad
    &\mathcal S=\mathbb F,\quad E_f=V_{\ell f},\quad T=h,
    \label{eq:common_recoveries:lac}\\
    \text{SNE:}\quad
    &\mathcal S=\mathcal N_i,\quad E_j=\|x_i-x_j\|_2^2,\quad T=2\sigma_i^2.
    \label{eq:common_recoveries:sne}
\end{empheq}
\end{subequations}
\end{proposition}

\begin{proof}
Let $p^\star$ be given by Eq.\,\eqref{eq:common_variational_minimizer} and define
\begin{equation}
    Z_T:=\sum_{b=1}^{m}\exp\left(-E_b/T\right).
    \label{eq:common_variational_partition}
\end{equation}
For any $p\in\Delta_{m-1}$, direct substitution into the Kullback--Leibler divergence gives
\begin{equation}
    \mathcal J_T[p]
    =-T\ln\left(Z_T\right)
     +T\KL\left(p\middle\|p^\star\right).
    \label{eq:common_variational_kl_decomposition}
\end{equation}
Nonnegativity of the divergence, Ref.\,\cite{kullback1951}, makes $p^\star$ the unique minimizer. Substitution of Eq.\,\eqref{eq:common_recoveries:lac} recovers the LAC weight distribution, while Eq.\,\eqref{eq:common_recoveries:sne} recovers the SNE conditional distribution. Proposition\,\ref{prop:common_entropy_principle} is therefore a specialization of the classical principle, not a new Gibbs theorem.
\end{proof}
...
\section{Prospective dual feature--neighbor construction}
\label{sec:dual}
...
For SNE, the response has a direct numerical use rather than only a thermodynamic interpretation. It converts a target perplexity accuracy into an anchor-specific first-order bandwidth tolerance, while controlled scaling experiments distinguish an exact self-similar law from regression and geometric effects present in Wine. The generalized Student discussion is deliberately limited to a conditional quadratic-energy representation and is not treated as an independent empirical result.
