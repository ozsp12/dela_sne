...
def select_cluster_response_h(V: np.ndarray, h_grid: np.ndarray = H_GRID) -> np.ndarray:
    return np.asarray([
        h_grid[np.argmax(response_curve(V[ell], h_grid))]
        for ell in range(V.shape[0])
    ])
...
def make_structured_synthetic(seed: int = SEED):
    """Three clusters with 2, 4, and 6 relevant coordinates and distinct contrasts."""
...
def alternating_lac_fixed_h(X, K, h, init_labels, max_iter=MAX_ITER):
    return _iterate_with_temperature_rule(
        X, K, init_labels, lambda V, it: float(h), max_iter
    )


def alternating_lac_meanV(X, K, init_labels, max_iter=MAX_ITER):
    return _iterate_with_temperature_rule(
        X, K, init_labels, lambda V, it: float(np.mean(V)), max_iter
    )
...
def alternating_lac_response_adaptive(X, K, init_labels, h_grid=H_GRID, max_iter=MAX_ITER):
    return _iterate_with_temperature_rule(
        ...
        K,
        init_labels,
        lambda V, it: select_global_response_h(V, h_grid),
        max_iter,
    )
...
def alternating_lac_response_clusterwise(X, K, init_labels, h_grid=H_GRID, max_iter=MAX_ITER):
    return _iterate_with_temperature_rule(
        ...
        K,
        init_labels,
        lambda V, it: select_cluster_response_h(V, h_grid),
        max_iter,
    )


def two_pass_response_lac(X, K, init_labels, h_grid=H_GRID, max_iter=MAX_ITER):
    """Select one global h from the initial partition and then freeze it."""
...
def evaluate_seed(seed: int, standardize: bool = False):
    X, y, _ = make_structured_synthetic(seed)
    if standardize:
    ...
    labels, _, it, status, _, _ = alternating_lac_fixed_h(X, K, 1.0, init)
    out["fixed_h1"] = adjusted_rand_score(y, labels)
    out["fixed_h1_iter"] = it
    out["fixed_h1_status"] = status

    labels, hmean, _, _, _, _ = alternating_lac_meanV(X, K, init)
    ...
    out["adaptive_h_unique"] = len(
        np.unique(np.asarray([float(np.asarray(h)) for h in hpath]))
    )
    ...
    if hi > lo:
        out["frozen_plateau_log_position"] = (
            np.log(out["frozen_h"]) - np.log(lo)
        ) / (np.log(hi) - np.log(lo))
    else:
...
def multiseed_lac_validation():
    rows = [
        evaluate_seed(SEED + s, standardize=standardized)
    ...
    for ell in range(K_SYN):
        axes[0].plot(
            h_dense,
            D0[ell],
            label=f"cluster {ell + 1}: {len(relevant0[ell])} relevant",
        )
    ...
    for ell in range(K_SYN):
        axes[1].plot(
            h_dense,
            C0[ell],
            label=rf"cluster {ell + 1}, $h^*={hstar0[ell]:.3f}$",
        )
    ...
    fig.tight_layout()
    fig.savefig(OUT / "lac_multiseed_validation.pdf", bbox_inches="tight")
    plt.close(fig)

    fields = [k for k in raw[0].keys() if k != "curve"]
    with open(OUT / "lac_multiseed_results.csv", "w", newline="") as fh:
    ...
    conv_fields = [
        ...
        "adaptive_iter",
        "adaptive_status",
        "adaptive_cycle_period",
        "adaptive_h_unique",
        "frozen_iter",
        "frozen_status",
        "frozen_cycle_period",
    ]
    with open(OUT / "lac_convergence_results.csv", "w", newline="") as fh:
...
def real_dataset_benchmarks():
    ...
    for name, X, y in datasets:
        ...
        for s in range(N_SEEDS):
            ...
            rows.append(row)
    with open(OUT / "lac_real_benchmarks.csv", "w", newline="") as fh:
...
def deterministic_annealing_diagnostic(X: np.ndarray, seed: int = SEED):
    ...
    for q, T in enumerate(temperatures):
        ...
        for _ in range(1000):
            ...
            if np.max(np.linalg.norm(Z_new - Z, axis=1)) < 1e-10:
                ...
                break
            Z = Z_new
        order_parameter[q] = np.sqrt(
            np.mean(np.sum((Z - Z.mean(axis=0)) ** 2, axis=1))
        )
    ...
    ax.invert_xaxis()
    ax.set_xscale("log")
    ax.set_ylabel("assignment order parameter")
    ax.set_xlabel("deterministic-annealing temperature")
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(OUT / "annealing_bifurcation.pdf", bbox_inches="tight")
    plt.close(fig)
...
def beta_for_perplexity(energies: np.ndarray, perplexity: float, tol: float = 1e-12):
    ...
    for _ in range(200):
        ...
        if entropy > target:
            lo = beta
            beta = np.sqrt(beta * hi) if hi < 1e11 else 2.0 * beta
        else:
            hi = beta
            beta = np.sqrt(beta * lo) if lo > 1e-11 else 0.5 * beta
    return beta, p
...
def local_temperatures(X: np.ndarray, perplexity: float):
    ...
    for i in range(n):
        ...
        T[i] = 1.0 / beta
        mean = np.dot(p, energies)
        var = np.dot(p, (energies - mean) ** 2)
        C[i] = var / T[i] ** 2
    return T, C, D
...
def density_proxy(D: np.ndarray, k: int):
    ...
    rho = np.zeros(n)
    for i in range(n):
        d = np.sort(D[i, np.arange(n) != i])
        rho[i] = 1.0 / np.mean(d[:k])
    return rho
...
def bootstrap_slope_ci(x, y, slope_fn, seed=SEED, B=3000):
    ...
    for b in range(B):
        ...
        slopes[b] = slope_fn(x[idx], y[idx])
    return np.quantile(slopes, [0.025, 0.975])


def controlled_scaling_diagnostic(perplexity: float = 20.0):
    rng = np.random.default_rng(2026)
    base = rng.uniform(-0.5, 0.5, size=(240, 5))
    scales = np.array([0.6, 0.8, 1.0, 1.3, 1.7, 2.2])
    med_T, med_rho = [], []
    for lam in scales:
        T, _, D = local_temperatures(lam * base, perplexity)
        rho = density_proxy(D, 20)
        med_T.append(np.median(T))
        med_rho.append(np.median(rho))
    med_T = np.asarray(med_T)
    med_rho = np.asarray(med_rho)
    x = np.log(med_rho)
    y = np.log(med_T)
    exact_slope = np.polyfit(x, y, 1)[0]
    rng = np.random.default_rng(314159)
    rep = 80
    xrep = np.repeat(x, rep)
    yrep = np.repeat(y, rep)
    noise_sd = 0.18
    xn = xrep + rng.normal(scale=noise_sd, size=len(xrep))
    yn = yrep + rng.normal(scale=noise_sd, size=len(yrep))
    return (
        med_rho,
        med_T,
        exact_slope,
...
def wine_neighbor_diagnostics(perplexity: float = 20.0):
    ...
    rho10 = density_proxy(D, 10)
    rho20 = density_proxy(D, 20)
    rho30 = density_proxy(D, 30)
    ...
    tls10 = tls_slope(x10, y)
    tls_ci10 = bootstrap_slope_ci(x10, y, tls_slope, seed=SEED + 1)
    ols20 = np.polyfit(np.log(rho20), y, 1)[0]
    tls20 = tls_slope(np.log(rho20), y)
    ols30 = np.polyfit(np.log(rho30), y, 1)[0]
    tls30 = tls_slope(np.log(rho30), y)

    control_rho, control_T, control_slope, xn, yn, noisy_ols, noisy_tls = (
        controlled_scaling_diagnostic(perplexity)
    )

    fig, axes = plt.subplots(3, 1, figsize=(7.1, 9.0))
    cfit = np.polyfit(np.log(control_rho), np.log(control_T), 1)
    xx = np.linspace(np.log(control_rho).min(), np.log(control_rho).max(), 200)
    axes[0].scatter(control_rho, control_T, s=30)
    axes[0].plot(
        np.exp(xx),
        np.exp(cfit[0] * xx + cfit[1]),
        label=rf"exact-copy slope $={control_slope:.3f}$",
    )
    axes[0].set_xscale("log")
    axes[0].set_yscale("log")
    axes[0].set_ylabel(r"median $T_i^{(N)}$")
    axes[0].set_xlabel(r"median density proxy $\rho_i^{(20)}$")
    axes[0].legend(frameon=False)

    axes[1].scatter(np.exp(xn), np.exp(yn), s=8, alpha=0.35)
    axes[1].set_xscale("log")
    axes[1].set_yscale("log")
    axes[1].set_xlabel("controlled noisy density proxy")
    axes[1].set_ylabel("controlled noisy temperature")
    axes[1].text(
        0.03,
        0.05,
        rf"OLS={noisy_ols:.3f}; TLS={noisy_tls:.3f}",
        transform=axes[1].transAxes,
    )

    axes[2].scatter(rho10, T, s=18, alpha=0.75)
    fit = np.polyfit(x10, y, 1)
    xx = np.linspace(x10.min(), x10.max(), 200)
    axes[2].plot(
        np.exp(xx),
        np.exp(fit[0] * xx + fit[1]),
        label=rf"Wine OLS slope $={ols10:.3f}$",
    )
    axes[2].set_xscale("log")
    axes[2].set_yscale("log")
    axes[2].set_xlabel(r"Wine density proxy $\rho_i^{(10)}$")
    axes[2].set_ylabel(r"local temperature $T_i^{(N)}$")
    axes[2].legend(frameon=False)
    fig.tight_layout()
    fig.savefig(OUT / "sne_density_scaling.pdf", bbox_inches="tight")
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(7.1, 4.4))
    ax.hist(C, bins=22, edgecolor="black", linewidth=0.45)
    q50, q90, q95 = np.quantile(C, [0.5, 0.9, 0.95])
    ax.axvline(q50, linewidth=1.0, label=rf"median $={q50:.3f}$")
    ax.axvline(
        q95,
        linewidth=1.0,
        linestyle="--",
        label=rf"95th percentile $={q95:.3f}$",
    )
    ax.set_xlabel(r"neighbor response $C_i^{(N)}$")
    ax.set_ylabel("anchors")
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(OUT / "sne_neighbor_response.pdf", bbox_inches="tight")
    plt.close(fig)
    ...
    return dict(
        ...
        ols10=ols10,
        ols10_ci_low=ci10[0],
        ols10_ci_high=ci10[1],
        tls10=tls10,
        tls10_ci_low=tls_ci10[0],
        tls10_ci_high=tls_ci10[1],
        ols20=ols20,
        tls20=tls20,
        ols30=ols30,
        tls30=tls30,
...
def summarize(raw, std, real_rows, hstar0, critical_T, observed_split, wine):
    ...
    for prefix, rows in [("raw", raw), ("std", std)]:
        for m in methods:
            a = arr(rows, m)
            summary[f"lac_{prefix}_{m}_mean"] = a.mean()
            summary[f"lac_{prefix}_{m}_sd"] = a.std(ddof=1)
        ...
        summary[f"lac_{prefix}_adaptive_cycle_fraction"] = np.mean(
            [r["adaptive_status"] == "cycle" for r in rows]
        )
        summary[f"lac_{prefix}_adaptive_maxiter_fraction"] = np.mean(
            [r["adaptive_status"] == "max_iter" for r in rows]
        )
        summary[f"lac_{prefix}_frozen_iter_mean"] = arr(rows, "frozen_iter").mean()
        summary[f"lac_{prefix}_frozen_cycle_fraction"] = np.mean(
            [r["frozen_status"] == "cycle" for r in rows]
        )
        summary[f"lac_{prefix}_plateau_factor_median"] = np.median(
            arr(rows, "plateau_factor")
        )
        summary[f"lac_{prefix}_adaptive_in_plateau_fraction"] = arr(
            rows, "adaptive_in_plateau"
        ).mean()
        summary[f"lac_{prefix}_frozen_in_plateau_fraction"] = arr(
            rows, "frozen_in_plateau"
        ).mean()

    comparisons = [
        (arr(raw, "adaptive_response"), arr(raw, "frozen_response"), "adaptive_minus_frozen"),
        (arr(raw, "adaptive_response"), arr(raw, "oracle_grid"), "adaptive_minus_oracle"),
        (arr(raw, "frozen_response"), arr(raw, "oracle_grid"), "frozen_minus_oracle"),
        (arr(raw, "adaptive_response"), arr(raw, "fixed_h1"), "adaptive_minus_h1"),
    ]
    for a, b, name in comparisons:
        d, ci = paired_bootstrap_mean_ci(a, b)
        summary[f"lac_{name}_mean"] = d
        summary[f"lac_{name}_ci_low"] = ci[0]
        summary[f"lac_{name}_ci_high"] = ci[1]

    for ell, h in enumerate(hstar0, 1):
        summary[f"representative_hstar_cluster_{ell}"] = h
    summary["deterministic_annealing_Tc_theory"] = critical_T
    summary["deterministic_annealing_first_observed_split"] = observed_split
    for k, v in wine.items():
        summary[f"wine_{k}"] = v

    X0, y0, rel = make_structured_synthetic(SEED)
    _, V0 = dispersions_from_labels(X0, y0, K_SYN)
    ratios = []
    for ell in range(K_SYN):
        mask = np.zeros(X0.shape[1], dtype=bool)
        mask[rel[ell]] = True
        ratios.append(V0[ell, ~mask].mean() / V0[ell, mask].mean())
    summary["synthetic_irrelevant_to_relevant_variance_ratio_min"] = min(ratios)
    summary["synthetic_irrelevant_to_relevant_variance_ratio_max"] = max(ratios)
    ...
    for ds in sorted(set(r["dataset"] for r in real_rows)):
        ...
        for m in [
            ...
            a = np.asarray([r[m] for r in rs])
            summary[f"real_{safe}_{m}_mean"] = a.mean()
            summary[f"real_{safe}_{m}_sd"] = a.std(ddof=1)

    with open(OUT / "numerical_summary.csv", "w", newline="") as fh:
        wr = csv.writer(fh)
        wr.writerow(["metric", "value"])
        for k in sorted(summary):
            wr.writerow([k, f"{summary[k]:.12g}"])
    return summary
...
def main():
    ...
    for m in [
        ...
        a = np.asarray([r[m] for r in raw])
        print(m, a.mean(), a.std(ddof=1))

    print(
        "Adaptive statuses",
        {
            s: sum(r["adaptive_status"] == s for r in raw)
            for s in ["converged", "cycle", "max_iter"]
        },
    )
    print(
        "Frozen statuses",
        {
            s: sum(r["frozen_status"] == s for r in raw)
            for s in ["converged", "cycle", "max_iter"]
        },
    )
