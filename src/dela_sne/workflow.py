"""Joint reproducible workflow for canonical LAC and t-SNE implementations."""
...
def default_test_dir() -> Path:
    return Path.cwd() / "data" / "test"


def default_result_dir() -> Path:
    return Path.cwd() / "data" / "result"
...
def run_dataset(
    ...
    destination = output_dir or default_result_dir()
    output_path = destination / f"{input_path.stem}_result.csv"
